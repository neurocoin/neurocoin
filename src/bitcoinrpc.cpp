...
// Copyright (c) 2017 NeuroCoin developers
...
{
    printf("JSONRPCError: code: %d, message: %s\n", code, message.c_str());
    Object error;
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop neurocoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "neurocoin server stopping";
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new neurocoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current neurocoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <neurocoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid neurocoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <neurocoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <neurocoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <neurocoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <neurocoinaddress> [minconf=1]\n"
            "Returns the total amount received by <neurocoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toneurocoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid neurocoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; neurocoin server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <neurocoinaddress>\n"
            "Return information about <neurocoinaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "neurocoin is not connected!"); //

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "neurocoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        if (modeval.type() == str_type)
            ...
            throw JSONRPCError(-8, "Invalid mode"); // xxxxx
    }
...
{
    bool paramsOk = params.size() >= 1 && params.size() <= 2;
    bool noParams = params.size() == 0;

    if (noParams) {
        // XXX unfortunately NOMP sends us `submitblock` without params when initiating a connection
        // let's try answering peacefully
        return Value::null;
    } else if (fHelp || !paramsOk)
        throw runtime_error(
    ...
    if (!block.SignBlock(*pwalletMain)) {
        // XXX temporarily disabled exception throwing here because the stratum python pool is not fully prepared for that
        printf("Unable to sign block, wallet locked?\n");
        //throw JSONRPCError(-100, "Unable to sign block, wallet locked?");
    }
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    ...
    try {
        ...
        //::Unserialize(&ssData, tx, SER_NETWORK, PROTOCOL_VERSION);
    }
    catch (std::exception &e) {
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: neurocoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static Value processSingleRequest(const Object& request, Value& id) {
    // Parse method
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use neurocoind";
    ...
    loop
    {
        ...
        try
        {
            // Parse request
            Value valRequest;
            printf("ThreadRPCServer payload: %s\n", strRequest.c_str());
            bool readed = read_string(strRequest, valRequest);
            if (!readed) {
            ...
            if (valRequest.type() == obj_type) {
                ...
                // single request (a regular case)

                const Object &request = valRequest.get_obj();

                // Parse id now so errors from here on will have the id
                id = find_value(request, "id");
                Value result = processSingleRequest(request, id);

                // Send reply
                string strReply = JSONRPCReply(result, Value::null, id);
                printf("ThreadRPCServer reply: %s\n", strReply.c_str());
                ...
                Array result;
                const Array &requests = valRequest.get_array();
                for (unsigned int i = 0; i < requests.size(); i++) {
                    Value aRequest = requests[i];
                    // todo check aRequest is really an object
                    Object &requestObj = aRequest.get_obj();
                    id = find_value(requestObj, "id");
                    Value aResp = processSingleRequest(requestObj, id);
                    result.push_back(aResp);
                }
                // Send reply
                string strReply = JSONRPCReply(result, Value::null, id);
                printf("ThreadRPCServer reply: %s\n", strReply.c_str());
                ...
                throw JSONRPCError(-32700, "Parse error - a JSON object/array expected");
            }
        }
