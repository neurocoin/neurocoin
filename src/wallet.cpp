...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    ...
    {
        if (nValue < 0) {
            printf("assertion failed: nValue < 0 (in loop): %lld\n", nValue);
            return false;
        }
        nValue += s.second;
    }
    if (vecSend.empty() || nValue < 0) {
        printf("assertion failed: vecSend.empty()\n");
        printf("assertion failed: nValue < 0: %lld\n", nValue);
        return false;
    }
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (nBytes >= MAX_BLOCK_SIZE_GEN/5) {
                    printf("assert failed: nBytes >= MAX_BLOCK_SIZE_GEN/5\n");
                    printf("nBytes: %u\n", nBytes);
                    printf("MAX_BLOCK_SIZE_GEN: %u\n", MAX_BLOCK_SIZE_GEN);
                    return false;
                }
                dPriority /= nBytes;
