...
bool CTransaction::CheckTransaction() const
{
    ...
    if (IsCoinBase())
    {
        unsigned long ss = vin[0].scriptSig.size();
        if (ss < 2 || ss > 120) {
            printf("vin[0].scriptSig.size(): %d\n", ss);
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    while (bnLowerBound + CENT <= bnUpperBound)
    {
        ...
        //CBigNum bnMidValueX4 = bnMidValue * bnMidValue * bnMidValue * bnMidValue;
        //CBigNum bnSubsidyLimitX4 = bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit;
        CBigNum bnMidValueX8 = bnMidValue * bnMidValue * bnMidValue * bnMidValue * \
                               bnMidValue * bnMidValue * bnMidValue * bnMidValue;
        CBigNum bnSubsidyLimitX8 = bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * \
                                   bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit;
        if (bnMidValueX8 * bnTargetLimit > bnSubsidyLimitX8 * bnTarget) {
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "ScienceDaily, May 15, 2017 Molecular dynamics, machine learning create 'hyper-predictive' computer models";
        CTransaction txNew;
        txNew.nTime = 1495022700;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1495022701;
        block.nBits    = bnProofOfWorkLimit.GetCompact(); // 486604799
        block.nNonce   = 1053298969;

        fprintf(stdout, "block.nBits: %u (unsigned), %d (signed)\n", block.nBits, block.nBits);
        ...
        fprintf(stdout, "block.hashMerkleRoot: %s\n", block.hashMerkleRoot.ToString().c_str());
        fprintf(stdout, "hashGenesisBlock: %s\n", hashGenesisBlock.ToString().c_str());
        fprintf(stdout, "block.GetHash(): %s\n", block.GetHash().ToString().c_str());

        assert(block.hashMerkleRoot == uint256("0x2a30cd74b5e9c07284a3dbdc3c8037b9a5eb0a93ca9e437b8dbe3e87bfaf22de"));

        /*
        CBigNum bnTarget;
        bnTarget.SetCompact(486604799);
...
string GetWarnings(string strFor)
{
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(PERIOD_TO_CONSIDER_LAST_CHECKPOINT_AS_OUTDATED_SECONDS) && !fTestNet)
    {
        nPriority = 100;
    ...
    if (strFor == "statusbar") {
        printf("warning for statusbar: %s", strStatusBar.c_str());
        return strStatusBar;
    } else if (strFor == "rpc") {
        printf("warning for RPC: %s", strRPC.c_str());
        return strRPC;
    }
