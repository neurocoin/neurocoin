...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\NeuroCoin
    // Mac: ~/Library/Application Support/NeuroCoin
    // Unix: ~/.neurocoin
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "NeuroCoin";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "neurocoin.conf"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong NeuroCoin will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "NeuroCoin:" << FormatVersion(NEUROCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
